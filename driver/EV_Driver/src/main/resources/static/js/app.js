...
async function loadCPs(){
  ...
  const sel = $('#cpSelect');
  if (sel){
    sel.innerHTML = data.map(c => `<option value="${c.uid}">${c.uid} — ${c.name||c.location||''}</option>`).join('');
  }
}
...
async function sendRequest(){
  ...
  if(!driverId || !cpUid){ alert('Set driverId and choose a CP'); return; }
...
async function checkRequest(){
  ...
  if(!id){ alert('Enter requestId'); return; }
...
async function startSim(){
  ...
  const driverId = Number($('#driverId').value || 0);
  if(!filePath || !driverId){ alert('Set file path and driverId'); return; }
